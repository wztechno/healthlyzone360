...
# Real mail is opt-in per instance: put BREVO_API_KEY (and optionally
# MAIL_FROM_ADDRESS / MAIL_FROM_NAME, which must be a sender Brevo has verified)
# into this directory's write-once .env and the instance sends through Brevo.
# Without the key mail is written to storage/logs/mail.log, so an instance
# nobody configured cannot email a real person by accident.
# The value of one .env key, surrounding quotes stripped.
envval() { local v; v="$(sed -n "s/^$1=//p" .env)"; v="${v%\"}"; v="${v#\"}"; v="${v%\'}"; v="${v#\'}"; printf '%s' "$v"; }
BREVO_API_KEY="$(envval BREVO_API_KEY)"
MAIL_FROM_ADDRESS="$(envval MAIL_FROM_ADDRESS)"
MAIL_FROM_NAME="$(envval MAIL_FROM_NAME)"
if [ -n "$BREVO_API_KEY" ]; then MAIL_MAILER=brevo; else MAIL_MAILER=log; fi
...
# Brevo when this instance's .env carries BREVO_API_KEY, otherwise the mail log.
