...
describe('the steps', () => {
    ...
    it('keeps production waste beside the yield on a route with no Packaging step', async () => {
        const stored = recipe({ ordinal: 5, name: 'Mujaddara' });

        await renderStubScreen(<RecipeEditScreen recipe={String(stored.id)} withoutPackaging />, {
            session: kitchenManagerSession(),
            repositories: { kitchenAdmin: editorReads(() => stored) },
        });

        await untilVisible('kitchen-recipe-editor-screen-header');
        expect(screen.queryByTestId('kitchen-recipe-tab-packaging')).toBeNull();
        expect(screen.getByTestId('kitchen-recipe-tab-steps-counter')).toHaveTextContent(
