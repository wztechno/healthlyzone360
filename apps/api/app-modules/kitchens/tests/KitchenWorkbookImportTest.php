...
/*
|--------------------------------------------------------------------------
| The private Healthy360 kitchen workbook importer, end to end (K1.8)
|--------------------------------------------------------------------------
|
...
function runImport(bool $dryRun = false, bool $validateOnly = false): ImportReport
{
    ...
    app(KitchenWorkbookImport::class)->run($options, $report);
...
function workbookKitchenOrganisation(): Organisation
{
    return Organisation::query()->where('slug', 'healthy360-kitchen')->sole();
}
 ...
 * needs afterwards.
 */
function inWorkbookKitchenContext(Closure $work): mixed
{
    $organisationId = (string) workbookKitchenOrganisation()->getKey();
...
it('stands the whole Healthy360 workbook kitchen up from the workbook', function (): void {
    $report = runImport();

    $organisation = workbookKitchenOrganisation();

    expect($organisation->name)->toBe(KitchenWorkbookWorld::ORGANISATION_NAME)
    ...
    inWorkbookKitchenContext(function (string $organisationId): void {
        ...
        expect($branch->name)->toBe(KitchenWorkbookWorld::BRANCH_NAME)
        ...
        expect(SalesChannel::query()->pluck('code')->sort()->values()->all())
            ->toBe([KitchenWorkbookWorld::CHANNEL_B2C, KitchenWorkbookWorld::CHANNEL_B2B])
            ->and(PriceList::query()->pluck('code')->sort()->values()->all())
            ->toBe([
                KitchenWorkbookWorld::PRICE_LIST_B2B,
                KitchenWorkbookWorld::PRICE_LIST_B2C,
                KitchenWorkbookWorld::PRICE_LIST_PLANS,
            ]);
    ...
    inWorkbookKitchenContext(function (string $organisationId): void {
        ...
        foreach ($tenantIngredients as $ingredient) {
            expect($ingredient->verification_status->value)->toBe('unverified')
                ->and($ingredient->source_system)->toBe('healthy360_workbook')
...
it('turns a dual pack into two variants and a single-priced dual pack into one price', function (): void {
    ...
    inWorkbookKitchenContext(function (): void {
...
it('claims every Lebanese area in one zone with no fee and no minimum', function (): void {
    ...
    inWorkbookKitchenContext(function (string $organisationId): void {
        ...
        expect($zone->code)->toBe('healthy360-delivery')
...
it('never overwrites an operator edit', function (): void {
    ...
    [$lineId, $priceId, $ingredientId] = inWorkbookKitchenContext(function (string $organisationId): array {
    ...
    inWorkbookKitchenContext(function () use ($lineId, $priceId, $ingredientId): void {
...
it('refuses to run outside the allowlisted environments', function (): void {
    ...
    $this->artisan('kitchen:import-workbook', ['--source' => fixtureWorkbook()])
...
it('refuses a source folder that is missing a workbook export', function (): void {
    $incomplete = sys_get_temp_dir().'/healthy360-incomplete-'.bin2hex(random_bytes(4));
    mkdir($incomplete);
    file_put_contents($incomplete.'/Actual Data_Recipes.md', "Sheet1:Technical Sheet\n");

    $options = new ImportOptions($incomplete, 'healthy360-kitchen');

    expect(fn () => app(KitchenWorkbookImport::class)->run($options, new ImportReport($options, 'testing')))
        ->toThrow(RuntimeException::class, 'missing');

    expect(Organisation::query()->where('slug', 'healthy360-kitchen')->exists())->toBeFalse();
});

it('runs from the console and writes a JSON report', function (): void {
    $this->artisan('kitchen:import-workbook', ['--source' => fixtureWorkbook()])
        ->expectsOutputToContain('Healthy360 workbook import')
        ->assertExitCode(0);

    expect(Organisation::query()->where('slug', 'healthy360-kitchen')->exists())->toBeTrue();
...
it('audits the run at both ends without carrying the findings into the audit trail', function (): void {
    $this->artisan('kitchen:import-workbook', ['--source' => fixtureWorkbook()])->assertExitCode(0);

    $events = DB::table('audit_logs')
        ->whereIn('action', ['catalogue.workbook_import_started', 'catalogue.workbook_import_finished'])
        ->get();

    expect($events)->toHaveCount(2);

    $finished = $events->firstWhere('action', 'catalogue.workbook_import_finished');
