...
/**
 * The private Healthy360 kitchen workbook importer's front door (master plan v2 §4.11).
 *
 ...
 */
final class ImportKitchenWorkbookCommand extends Command
{
    protected $signature = 'kitchen:import-workbook
    ...
    public function handle(KitchenWorkbookImport $import): int
    {
        ...
        if (! in_array($environment, $allowed, true)) {
            $this->components->error(sprintf(
                'kitchen:import-workbook refuses to run in the "%s" environment.',
                $environment,
            ));
            $this->line('');
            ...
            $this->line('');
            $this->line('  Widen kitchens.import.environments (or KITCHEN_WORKBOOK_IMPORT_ENVIRONMENTS) deliberately,');
        ...
        $options = new ImportOptions(
            ...
            organisationSlug: $this->stringOption('org') ?? (string) config('kitchens.import.organisation_slug', 'healthy360-kitchen'),
        ...
        $this->components->info(sprintf(
            'Healthy360 workbook import — mode: %s · organisation: %s · environment: %s',
            $options->mode(),
