...
final class PurchaseOrderShowController
{
    ...
    {
        $order = PurchaseOrder::query()->with(['supplier', 'branch', 'lines'])->whereKey($purchaseOrder)->first();

        if ($order === null) {
            throw new ApiException(ErrorCode::ResourceNotFound);
        }

        return ApiResponse::data(['purchase_order' => $this->presenter->purchaseOrder($order)]);
    }
}
