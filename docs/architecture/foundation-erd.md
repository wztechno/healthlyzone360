...
- **Successor note.** This document describes the foundation's 23 tables and stops there. The kitchen-programme tables land per master plan v2, one phase at a time; each one's **per-module migration docblock is authoritative** for its columns, constraints and the reasoning behind them, and `docs/registers/data-register.md` carries its classification and isolation strategy. Nothing here is renumbered as those phases land.
