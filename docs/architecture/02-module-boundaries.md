...
Permissions for those modules likewise remain registry proposals until they are implemented; the kitchen and commercial sets (`catalogue.*`, `recipe.*`, `plan.*`, `price_list.*`, `delivery_zone.*`) are real, held by real template roles, and pinned in three places: `PermissionRegistryTest` (what the registry declares — the role list, each role's grant count and who holds each code a role split turns on, all hard-coded), `RlsTest` (the template-role count under row-level security) and `tests/Feature/DatabaseSeederTest.php` (that the seeder wrote exactly the registry's roles and grants). The first two are hard-coded and have to move together; the third compares against the registry, so it follows on its own (D-141). Naming all three matters — the access console updated one, a follow-up updated a second, and the third sat red until it was swept.
