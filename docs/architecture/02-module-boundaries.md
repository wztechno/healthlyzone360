...
Permissions for those modules likewise remain registry proposals until they are implemented; the kitchen and commercial sets (`catalogue.*`, `recipe.*`, `plan.*`, `price_list.*`, `delivery_zone.*`) are real, held by real template roles, and pinned in three places that have to move together: `PermissionRegistryTest` (what the registry declares), `RlsTest` (the template-role count under row-level security) and `tests/Feature/DatabaseSeederTest.php` (what the seeder actually wrote). Naming all three matters — the access console updated one, a follow-up updated a second, and the third sat red until it was swept.
