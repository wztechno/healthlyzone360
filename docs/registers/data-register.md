...
**Tables carrying a PostgreSQL policy today (eight, pinned by `RlsTest`)**: `organisation_branches`, `organisation_memberships`, `roles`, `feature_entitlements`, `consent_grants`, `audit_logs` (foundation) and `recipe_versions`, `recipe_version_lines` (K1.2). Every new tenant-scoped table decides at migration time whether it joins the set — the pin exists so the decision is deliberate rather than inherited.
