...
function summarise(catalogue) {
  ...
  for (const [key, value] of flat) {
    ...
    const entry = bases.get(base) ?? { placeholders: new Set(), suffixes: new Set(), values: [] };
  ...
  for (const locale of locales) {
    ...
    for (const [base, referenceEntry] of reference) {
      ...
      if (referenceEntry.suffixes.size > 0) {
        ...
        const missingCategories = [...required].filter((category) => !entry.suffixes.has(category));
      ...
      if (
        ...
        warnings.push(`${locale}/${namespace}: "${base}" is identical to ${REFERENCE_LOCALE}`);
      }
    }

    for (const base of target.keys()) {
      if (!reference.has(base)) {
        errors.push(`${locale}/${namespace}: unknown key "${base}" (not present in ${REFERENCE_LOCALE})`);
      }
    }
  }
}
