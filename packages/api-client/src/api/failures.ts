...
import { apiFailure, rateLimitFailure, validationFailure } from '../contracts/failure.ts';
 ...
 *
 * Unmapped codes (`authz.permission_denied`, `resource.not_found`, `resource.conflict`,
 * `request.invalid`, `auth.invalid_signature`, `server.internal_error`) become `server` while
 * keeping the **server's own message and correlation identifier**, so the person still sees a true
 * sentence and support can still find the request. `retryable` follows the status class: repeating
 * a rejected `4xx` unchanged cannot help, a `5xx` might.
 *
 * None of those codes is reachable from an implemented repository call today. When one becomes
 * reachable, it earns a place in `API_FAILURE_CODES` rather than a wider `default:`.
 */
