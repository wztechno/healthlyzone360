...
import type { SelectProps } from './select-shared.ts';
 ...
 *
 * This is the half a phone gets, and the modal is the right answer there rather than a
 * compatibility fallback: an anchored panel on a 375px viewport is a list squeezed against the
 * bottom of the screen with the keyboard over it. The web half renders the same options in a
 * dropdown under the field — see `select.web.tsx`.
 *
 ...
 * dialog containing `role="radiogroup"` is unambiguously valid ARIA and is the native pattern
 * anyway. React Native's `Modal` traps focus and calls `onRequestClose` on the hardware back
 * gesture, so none of it has to be hand-rolled.
 *
